# In case this is needed to get rid of line endings on windows: sed -i 's/\r$//' backup.sh
...
/usr/local/bin/aws s3 cp $TARFILENAME s3://$BUCKET/ --storage-class STANDARD_IA --profile default
